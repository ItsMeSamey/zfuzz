...
const basic_cases = [_]Case{
    .{ .name = "version", .script = "\"$1\" --version", .stdout = "zfuzz 0.3.0\n" },
...
const interactive_cases = [_]Case{
    ...
    .{ .name = "pty default up", .script = "D=.zig-cache/e2e-up-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'one\\ntwo\\nthree\\n' > \"$D/in\"; { sleep .25; printf '\\033[A\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "two\n" },
    .{ .name = "pty default wheel up", .script = "D=.zig-cache/e2e-wheel-up-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; seq -f 'item-%02g' 1 30 > \"$D/in\"; { sleep .25; printf '\\033[<64;10;4M\\r'; } | timeout 3s script -qefc \"stty rows 8 cols 60; $1 --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "item-04\n" },
    .{ .name = "pty default wheel roundtrip", .script = "D=.zig-cache/e2e-wheel-roundtrip-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; seq -f 'item-%02g' 1 30 > \"$D/in\"; { sleep .25; printf '\\033[<64;10;4M\\033[<65;10;4M\\r'; } | timeout 3s script -qefc \"stty rows 8 cols 60; $1 --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "item-01\n" },
    .{ .name = "pty default page up", .script = "D=.zig-cache/e2e-page-up-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; seq -f 'item-%02g' 1 30 > \"$D/in\"; { sleep .25; printf '\\033[5~\\r'; } | timeout 3s script -qefc \"stty rows 8 cols 60; $1 --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "item-07\n" },
    .{ .name = "pty reverse down", .script = "D=.zig-cache/e2e-rdown-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'one\\ntwo\\nthree\\n' > \"$D/in\"; { sleep .25; printf '\\033[B\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 --reverse --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "two\n" },
    .{ .name = "pty reverse wheel down", .script = "D=.zig-cache/e2e-rwheel-down-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; seq -f 'item-%02g' 1 30 > \"$D/in\"; { sleep .25; printf '\\033[<65;10;4M\\r'; } | timeout 3s script -qefc \"stty rows 8 cols 60; $1 --reverse --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "item-04\n" },
    .{ .name = "pty reverse page down", .script = "D=.zig-cache/e2e-rpage-down-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; seq -f 'item-%02g' 1 30 > \"$D/in\"; { sleep .25; printf '\\033[6~\\r'; } | timeout 3s script -qefc \"stty rows 8 cols 60; $1 --reverse --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "item-07\n" },
    .{ .name = "pty ctrl-k", .script = "D=.zig-cache/e2e-ctrlk-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'one\\ntwo\\nthree\\n' > \"$D/in\"; { sleep .25; printf '\\013\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 --no-sort < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "two\n" },
    ...
    .{ .name = "pty expect key", .script = "D=.zig-cache/e2e-expect-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'one\\ntwo\\n' > \"$D/in\"; { sleep .25; printf '\\030'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 --no-sort --expect=ctrl-x < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "ctrl-x\none\n" },
};
const interactive_input_cases = [_]Case{
    .{ .name = "pty bracketed paste", .script = "D=.zig-cache/e2e-paste-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; { sleep .25; printf '\\033[200~ga\\033[201~\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "gamma\n" },
    .{ .name = "pty tiny terminal", .script = "D=.zig-cache/e2e-tiny-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; { sleep .25; printf '\\r'; } | timeout 3s script -qefc \"stty rows 2 cols 2; $1 --query=ga < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "gamma\n" },
    ...
    .{ .name = "pty lazy ASCII stream becomes Unicode", .script = "D=.zig-cache/e2e-lazy-ascii-unicode-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; { sleep .55; printf '\\r'; } | timeout 5s script -qefc \"stty rows 12 cols 60; (i=0; while [ \\$i -lt 12000 ]; do printf 'ordinary-%05d\\n' \\$i; i=\\$((i+1)); done; sleep .15; printf 'Danço\\n') | $1 --query=danco > $D/out\" /dev/null >/dev/null 2>&1; rc=$?; [ $rc -eq 0 ]; cat \"$D/out\"", .stdout = "Danço\n" },
    .{ .name = "pty lazy accept waits through empty partial", .script = "D=.zig-cache/e2e-lazy-empty-partial-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; { sleep .25; printf '\\r'; } | timeout 4s script -qefc \"stty rows 12 cols 60; (i=0; while [ \\$i -lt 20000 ]; do printf 'ordinary-%05d\\n' \\$i; i=\\$((i+1)); done; sleep .45; printf 'gamma\\n') | $1 --query=gamma > $D/out\" /dev/null >/dev/null 2>&1; rc=$?; [ $rc -eq 0 ]; cat \"$D/out\"", .stdout = "gamma\n" },
    .{ .name = "pty lazy select-1 uses retained result", .script = "D=.zig-cache/e2e-lazy-select1-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; timeout 3s script -qefc \"stty rows 12 cols 60; $1 --select-1 --query=gamma < $D/in > $D/out\" /dev/null >/dev/null 2>&1; rc=$?; [ $rc -eq 0 ]; cat \"$D/out\"", .stdout = "gamma\n" },
    .{ .name = "pty query typing", .script = "D=.zig-cache/e2e-type-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; { sleep .25; printf 'ga\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "gamma\n" },
    .{ .name = "pty query refresh keeps cursor row", .script = "D=.zig-cache/e2e-stable-focus-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nab\\naxa\\nzebra\\na-long-long\\n' > \"$D/in\"; { sleep .25; printf '\\033[A'; sleep .03; printf 'a'; sleep .08; printf '\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "axa\n" },
    .{ .name = "pty redraw avoids repeated full screen clears", .script = "D=.zig-cache/e2e-redraw-clear-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; { sleep .25; printf 'g'; sleep .05; printf 'a'; sleep .05; printf '\\r'; } | timeout 3s script -qefc \"stty rows 12 cols 60; $1 < $D/in > $D/out\" \"$D/tty\" >/dev/null 2>&1; python3 -c 'import sys; d=open(sys.argv[1],\"rb\").read(); print(d.count(b\"\\x1b[2J\"))' \"$D/tty\"; cat \"$D/out\"", .stdout = "1\ngamma\n" },
    .{ .name = "pty lazy progress redraw stays clear-free", .script = "D=.zig-cache/e2e-lazy-redraw-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; { sleep .35; printf '\\033'; } | timeout 3s script -qefc \"stty rows 12 cols 60; (b=0; while [ \\$b -lt 6 ]; do i=0; while [ \\$i -lt 2000 ]; do printf 'alpha-%d-%04d\\n' \\$b \\$i; i=\\$((i+1)); done; sleep .03; b=\\$((b+1)); done) | $1 --query=a >/dev/null\" \"$D/tty\" >/dev/null 2>&1; rc=$?; [ $rc -eq 130 ]; python3 -c 'import sys; d=open(sys.argv[1],\"rb\").read(); assert d.count(b\"\\x1b[2J\")==1; assert d.count(b\"\\x1b[?2026h\")>=3; assert d.count(b\"lpha-0-0000\")==1; print(\"stable\")' \"$D/tty\"", .stdout = "stable\n" },
    .{ .name = "pty slow preview stays responsive", .script = "D=.zig-cache/e2e-preview-responsive-$$; mkdir -p \"$D\"; trap 'rm -rf \"$D\"' EXIT; printf 'alpha\\nbeta\\ngamma\\n' > \"$D/in\"; { sleep .25; printf 'ga\\r'; } | timeout 1s script -qefc \"stty rows 12 cols 60; $1 --preview='sleep 2; printf slow-preview' < $D/in > $D/out\" /dev/null >/dev/null 2>&1; cat \"$D/out\"", .stdout = "gamma\n" },
};
